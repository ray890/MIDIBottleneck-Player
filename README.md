...
There is no principal AnyCPU build. The output list uses the Windows MIDI output API, so it includes installed software synthesizers (such as Microsoft GS Wavetable Synth, when present) and physical/virtual MIDI devices. When OmniMIDI is installed, the adjacent **KDMAPI** checkbox selects its direct API instead; the ordinary output list is disabled because KDMAPI targets OmniMIDI itself. KDMAPI loads the native module matching the running executable and reports an explicit x86/x64 error if that module is absent or incompatible.
...
Opening a MIDI file is asynchronous and cancellable. The main window remains repaintable, shows the current stage plus two determinate progress bars, and temporarily turns **Open MIDI...** into **Cancel**. The upper bar is monotonic progress through byte-weighted track reading/parsing, stable merging, timestamp assignment, and finalization; the restrained lower bar is exact progress inside the current stage. The two bars share the normal top file row with the loading filename and replace the ordinary file summary, so MIDI output, Processing model, Playback, Statistics, and the window height never move merely because loading starts or ends. Progress reports are sampled by the existing UI timer rather than queued to the message pump one by one. All large loops check cancellation, and a cancelled or superseded operation cannot publish a partial or stale `MidiSong`.
...
The final merged event list still requires one contiguous reference array. Before allocating it, the parser checks the architecture-specific .NET Framework single-array byte ceiling and reports a clear contiguous-storage-limit error; it does not attempt an allocation known to be impossible. Supporting still larger files requires a future segmented event-store design rather than enabling huge arrays without auditing playback and Analysis assumptions.
